...
class Lutador implements ILutador { // Declaração da classe Lutador que implementa a interface ILutador
    ...
    public function apresentar() // Mostra na tela uma apresentação do lutador, com todas suas informações
    {
        ...
        <h2>{$this->getNome()}</h2>
        <p>-------------------------------</p>
        <p>Nacionalidade: {$this->getNacionalidade()}</p>
        <p>Idade: {$this->getIdade()}</p>
        <p>Altura: {$this->getAltura()}</p>
        <p>Peso: {$this->getPeso()}</p>
        <p>Categoria: {$this->getCategoria()}</p>
        <p>Vitorias: {$this->getVitorias()}</p>
        <p>Derrotas: {$this->getDerrotas()}</p>
        <p>Empates: {$this->getEmpates()}</p>
        ";
    }
